...
package xstar.com.kotlintest.zxing.camera;
...
//            cameraResolution = findBestPreviewSizeValue(previewSizeValueString, screenResolution);
    ...
    private static Point findBestPreviewSize(List<Camera.Size> sizeList, Point screenResolution) {
        float bili = getBili(screenResolution.x, screenResolution.y);
        float bili_ = 1;
        float temp_bili = 0;
        Camera.Size bestSize = null;
        for (Camera.Size size : sizeList) {
            temp_bili = bili - getBili(size.width, size.height);
            temp_bili = Math.abs(temp_bili);
            if (temp_bili < bili_) {
                bili_ = temp_bili;
                bestSize = size;
            }
    ...
    private static float getBili(int w, int h) {
        if (w > h) {
            return h / (float) w;
        }
        return w / (float) h;
